...
const TEMPLATES = [
    ...
    { id: "undangan", label: "📄 [UMUM] Surat Undangan", value: "undangan", content: "SURAT UNDANGAN<br><br>Kepada Yth,<br>[Nama Undangan]<br>di Tempat<br><br>Dengan hormat,<br><br>Kami mengundang Bapak/Ibu/Saudara/i untuk menghadiri acara:<br><br>Acara: [Nama Acara]<br>Hari/Tanggal: [Hari, Tanggal]<br>Waktu: [Jam]<br>Tempat: [Lokasi Lengkap]<br><br>Meningat pentingnya acara ini, kami sangat mengharapkan kehadiran Bapak/Ibu tepat waktu.<br><br>Demikian undangan ini kami sampaikan. Atas perhatian dan kehadirannya, kami ucapkan terima kasih.<br><br>[Tempat, Tanggal]<br><br>Hormat kami,<br><br><br>[Nama Penyelenggara]" },
...
const selectedCity = ref("Jakarta");
...
const getIndonesianDate = (city: string = selectedCity.value) => {
    const months = [
...
const handleFormat = (command: string, value?: string) => {
    ...
    if (editorRef.value) {
        updateState({ content: editorRef.value.innerHTML });
    }
};
...
watch(
    ...
    (newContent) => {
        if (editorRef.value) {
            ...
            if (editorRef.value.innerHTML !== normalizedStateContent) {
...
onMounted(() => {
    if (editorRef.value) {
        editorRef.value.innerHTML = state.value.content;
    }
...
<template>
    <div class="space-y-6">
        <div class="space-y-3">
            <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Isi Surat</label>
            <div class="bg-[#0f172a]/40 rounded-2xl border border-white/10 focus-within:border-[#0ea5e9]/50 transition-all duration-500 shadow-2xl backdrop-blur-md">
                <!-- Unified Toolbar -->
                <div class="flex items-center gap-1.5 md:gap-3 p-2 md:p-3 bg-white/[0.02] border-b border-white/5 flex-wrap rounded-t-2xl">
                    <!-- Action Group -->
                    <div class="flex items-center gap-0.5 bg-white/[0.03] rounded-lg p-0.5 border border-white/5 shrink-0">
                        <button type="button" @mousedown.prevent="handleFormat('undo')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Undo">
                            <Undo size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('redo')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Redo">
                            <Redo size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                    </div>

                    <!-- Format Group -->
                    <div class="flex items-center gap-0.5 bg-white/[0.03] rounded-lg p-0.5 border border-white/5 shrink-0">
                        <button type="button" @mousedown.prevent="handleFormat('bold')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Bold">
                            <Bold size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('italic')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Italic">
                            <Italic size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('underline')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Underline">
                            <Underline size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('strikeThrough')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Strikethrough">
                            <Strikethrough size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                    </div>

                    <!-- Alignment Group -->
                    <div class="flex items-center gap-0.5 bg-white/[0.03] rounded-lg p-0.5 border border-white/5 shrink-0">
                        <button type="button" @mousedown.prevent="handleFormat('justifyLeft')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Align Left">
                            <AlignLeft size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('justifyCenter')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Align Center">
                            <AlignCenter size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('justifyRight')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Align Right">
                            <AlignRight size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('justifyFull')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Justify">
                            <AlignJustify size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                    </div>

                    <!-- List Group -->
                    <div class="flex items-center gap-0.5 bg-white/[0.03] rounded-lg p-0.5 border border-white/5 shrink-0">
                        <button type="button" @mousedown.prevent="handleFormat('insertUnorderedList')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Bullet List">
                            <List size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('insertOrderedList')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Numbered List">
                            <ListOrdered size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                    </div>

                    <!-- Misc Group -->
                    <div class="flex items-center gap-0.5 bg-white/[0.03] rounded-lg p-0.5 border border-white/5 shrink-0">
                        <button type="button" @mousedown.prevent="handleFormat('insertHorizontalRule')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Horizontal Line">
                            <Minus size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                        <button type="button" @mousedown.prevent="handleFormat('removeFormat')" class="p-1.5 rounded-lg text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10 transition-all duration-300 active:scale-90 group" title="Clear Formatting">
                            <Eraser size="14" class="group-hover:scale-110 transition-transform duration-300" />
                        </button>
                    </div>

                    <!-- Date Tool -->
                    <div class="flex items-center bg-white/[0.03] rounded-lg p-0.5 border border-white/5 relative">
                        <button
                            type="button"
                            @mousedown.prevent="showDateOptions = !showDateOptions"
                            class="h-7 px-3 rounded-md text-[10px] font-bold transition-all flex items-center gap-2"
                            :class="showDateOptions ? 'bg-[#0ea5e9] text-white shadow-lg shadow-[#0ea5e9]/20' : 'text-slate-400 hover:text-[#0ea5e9] hover:bg-[#0ea5e9]/10'"
                            title="Insert Date"
                        >
                            <Calendar size="13" />
                        ...
                        <div v-if="showDateOptions" class="absolute left-0 top-10 z-50 bg-[#1e293b] border border-white/10 rounded-xl p-3 shadow-2xl min-w-[200px] space-y-2 backdrop-blur-xl">
                            <label class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Pilih Kota</label>
                            <select
                                v-model="selectedCity"
                                class="w-full bg-[#0f172a]/60 border border-white/10 rounded-lg px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-[#0ea5e9]/50"
                            >
                                <option v-for="city in cities" :key="city" :value="city">{{ city }}</option>
                            </select>
                            <div class="text-[10px] text-slate-400 mt-1 mb-1 bg-[#0f172a]/40 p-2 rounded-md border border-white/5">
                                <span class="opacity-50 block mb-0.5">Preview:</span>
                                <span class="text-[#0ea5e9] font-medium">{{ getIndonesianDate() }}</span>
                            </div>
                            <button
                                type="button"
                                @click="handleInsertDate"
                                class="w-full py-2 bg-[#0ea5e9] hover:brightness-110 text-white rounded-lg text-[10px] font-black transition-all active:scale-95 shadow-lg shadow-[#0ea5e9]/20"
                            >
                                Sisipkan Tanggal
                            </button>
                        </div>
                ...
                <div
                    ...
                    @input="updateState({ content: ($event.currentTarget as HTMLDivElement).innerHTML })"
                    class="editor-content w-full h-40 md:h-56 bg-transparent p-4 md:p-6 border-none focus:ring-0 overflow-auto text-sm outline-none text-slate-300 leading-relaxed custom-scrollbar"
        ...
        <div class="space-y-3">
            <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Template Cepat</label>
            <UiCustomSelect
                model-value=""
                @change="(val) => {
